...
inline bool is_supported_open_protocol(const std::string& url) { return is_meshforge_open(url) || is_prusaslicer_open(url) || is_bambustudio_open(url) || is_cura_open(url); }
