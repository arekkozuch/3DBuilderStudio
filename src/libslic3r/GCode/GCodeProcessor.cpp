...
void GCodeProcessor::run_post_process()
{
    ...
    auto process_placeholders = [&](std::string& gcode_line) {
        ...
        if (line.length() > 1) {
            ...
            // MeshForge: write total layer number, this is used by BBL-compatible printers only as of now
...
const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    //BBS: MeshForge matches the upstream producer string for time estimation compatibility.
    //FIXME: Workaround - should be refactored when upstream producer detection is overhauled
    ...
    //{ EProducer::Slic3rPE,    "generated by Slic3r (upstream edition)" },
...
void GCodeProcessor::process_G92(const GCodeReader::GCodeLine& line)
{
    ...
    if (!any_found && !line.has_unknown_axis()) {
        // The G92 may be called for axes that the slicer does not recognize, for example see GH issue #3510,
...
{
    //BBS: for BBL-compatible printers, we both use M106 P1 and M106 to indicate the part cooling fan
    ...
    for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i) {
        if (static_cast<PrintEstimatedStatistics::ETimeMode>(i) == PrintEstimatedStatistics::ETimeMode::Normal ||
            ...
            if (line.has_value('S', value)) {
                ...
                // It is also generated by upstream slicers to control acceleration per extrusion type
