    ...
    const bool grayscale/* = false*/, const bool dark_mode/* = false*/, const std::string& new_color /*= ""*/, const float scale_in_center/* = 0*/)
{
    ...
    if (dark_mode) {
        ...
        replaces["#DBDBDB"] = "#4A4A51"; // MeshForge border color
        replaces["#F0F0F1"] = "#333337"; // MeshForge disabled background color
        replaces["#262E30"] = "#EFEFF0"; // MeshForge
    } else {
        replaces["#949494"] = "#7C8282"; // MeshForge replace icon line color for light theme
    }

    if (strstr(bitmap_name.c_str(), "toggle_on") != NULL && dark_mode) // MeshForge only replace color of toggle button
        replaces["#009688"] = "#00675b";
