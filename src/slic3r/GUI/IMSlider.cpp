...
void IMSlider::SetSelectionSpan(const int lower_val, const int higher_val)
{
    ...
    // MeshForge reset single layer position when min max values changed
...
bool IMSlider::switch_one_layer_mode()
{
    ...
    if (!m_is_one_layer) {                       // DEACTIVATE
        m_one_layer_value = GetHigherValue();       // MeshForge Backup value on deactivate
        SetLowerValue(m_min_value);
        SetHigherValue(m_max_value);             // Higher value resets on toggling off one layer mode to show whole model
    }else{                                       // ACTIVATE
                                                 // MeshForge Ensure value fits range. value set in IMSlider::SetSelectionSpan but added this just in case
        ...
        else if(GetHigherValue() == m_max_value) // MeshForge Prefer backup value if higher value reseted
            SetHigherValue(m_one_layer_value);      // MeshForge Restore value
        else                                     // MeshForge Prefer higher value if user changed higher value. so it will show section on same view
            SetHigherValue(GetHigherValue());    // MeshForge use same position with higher value if user changed its position. visible section stays same when switching one layer mode with this
    }
...
bool IMSlider::render(int canvas_width, int canvas_height)
{
    ...
    ImGui::PushStyleColor(ImGuiCol_::ImGuiCol_Text, ImGuiWrapper::COL_ACCENT); // MeshForge: Use accent color for slider value text
    ...
    if (is_horizontal()) {
        ...
        if (evt.GetPosition().x > layers_slider_window->Pos.x &&
            ...
            if (is_one_layer()) {
                const int new_pos = GetHigherValue() + wheel;
                SetHigherValue(new_pos);
                m_one_layer_value = new_pos; // MeshForge backup value for single layer mode
            }
            else {
