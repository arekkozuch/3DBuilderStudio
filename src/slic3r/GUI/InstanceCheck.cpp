...
namespace instance_check_internal
{
	...
	{
		// checks for other instances of the same app — bring to front if found
		...
		if (wndTextString.find(L"MeshForge") != std::wstring::npos && classNameString == L"wxWindowNR") {
	...
	static bool  send_message(const std::string &message_text, const std::string &version)
	{
		...
		if ( !checker->IsAnotherRunning() ) */
		{
			...
			std::string		interface_name = "com.meshforge.MeshForge.InstanceCheck.Object" + version;
			std::string   	method_name = "AnotherInstance";
			std::string		object_name = "/com/meshforge/MeshForge/InstanceCheck/Object" + version;
...
void OtherInstanceMessageHandler::handle_message(const std::string& message) 
{
	...
	boost::regex re(R"(^(meshforge|prusaslicer|cura|bambustudio):\/\/open[\/]?\?file=)", boost::regbase::icase); // based on upstream URL scheme compatibility
	boost::regex re2(R"(^(bambustudioopen):\/\/)", boost::regex::icase); // based on upstream URL scheme compatibility
...
namespace MessageHandlerDBusInternal
{
	...
	static void respond_to_introspect(DBusConnection *connection, DBusMessage *request) 
	{
	    ...
	    const char  *introspection_data =
	        ...
	        "     </method>"
	        "   </interface>"
	        "   <interface name=\"com.meshforge.MeshForge.InstanceCheck\">"
	...
	//method AnotherInstance receives message from another MeshForge instance 
	...
	static DBusHandlerResult handle_dbus_object_message(DBusConnection *connection, DBusMessage *message, void *user_data)
	{
	    ...
	    std::string our_interface  = "com.meshforge.MeshForge.InstanceCheck.Object" + wxGetApp().get_instance_hash_string();
...
void OtherInstanceMessageHandler::listen()
{
	...
	std::string			 interface_name = "com.meshforge.MeshForge.InstanceCheck.Object" + instance_hash;
	std::string			 object_name 	= "/com/meshforge/MeshForge/InstanceCheck/Object" + instance_hash;
	...
	if (DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER != name_req_val) {
		BOOST_LOG_TRIVIAL(error) << "Not primary owner of DBus name - probably another MeshForge instance is running.";
	    BOOST_LOG_TRIVIAL(error) << "Dbus Messages listening terminating.";
	    dbus_connection_unref(conn);
	    return;
	}
