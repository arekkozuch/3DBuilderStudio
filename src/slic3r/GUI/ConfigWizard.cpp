...
BundleMap BundleMap::load()
{
    ...
    auto orca_bundle_path = (vendor_dir / PresetBundle::MESHFORGE_DEFAULT_BUNDLE).replace_extension(".json");
    auto orca_bundle_rsrc = false;
    if (!boost::filesystem::exists(orca_bundle_path)) {
        orca_bundle_path = (rsrc_vendor_dir / PresetBundle::MESHFORGE_DEFAULT_BUNDLE).replace_extension(".json");
        orca_bundle_rsrc = true;
    }
    {
        Bundle bbl_bundle;
        if (bbl_bundle.load(std::move(orca_bundle_path), orca_bundle_rsrc, true))
            res.emplace(PresetBundle::MESHFORGE_DEFAULT_BUNDLE, std::move(bbl_bundle));
    }
...
{
    // MeshForge: add custom as default
    auto it = find(PresetBundle::MESHFORGE_DEFAULT_BUNDLE);
    if (it == end()) {
        throw Slic3r::RuntimeError("ConfigWizard: Internal error in BundleMap: MESHFORGE_DEFAULT_BUNDLE not loaded");
    }
...
void PagePrinters::set_run_reason(ConfigWizard::RunReason run_reason)
{
    ...
    if (is_primary_printer_page
        ...
        && printer_pickers[0]->vendor_id == PresetBundle::MESHFORGE_DEFAULT_BUNDLE) {
...
ConfigWizardIndex::ConfigWizardIndex(wxWindow *parent)
    : wxPanel(parent)
    , bg(ScalableBitmap(parent, "MeshForge_192px_transparent.png", 192))
...
void ConfigWizard::priv::create_3rdparty_pages()
{
    for (const auto &pair : bundles) {
        ...
        // MeshForge: add custom as default
        if (vendor->id == PresetBundle::MESHFORGE_DEFAULT_BUNDLE) { continue; }
